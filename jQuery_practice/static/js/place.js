...
// 	}	
// };
$(document).on("click", "#goenroll", function(){
	if(userId == null){
		alert("로그인 후 이용 가능합니다. 로그인을 해주세요.");
		return false;
	}else{
...
$.ajax({
	...
	success: function(data){
		$("#pname").html(`[${data.theme}]` + " 	" + data.placeName + " 	" + data.detailAddress);
		...
		if(data.reviewList.length !== 0){
			for(let count=0; count<data.reviewList.length; count++){
				...
				let okBtn = `<button type=button id=okBtn value=${reviewId} class="btn btn-sm btn-secondary" style="margin-left: 20px; margin-right: 5px;">` + "확인" + "</button>";
				let cancleBtn = `<button type=button id=cancleBtn value=${reviewId} class="btn btn-sm btn-secondary">` + "취소" + "</button>";
				//수정버튼 클릭 시 hide				
				let modifyBtn = `<button type=button id=modifyBtn value=${reviewId} class="btn btn-sm btn-secondary" style="margin-left: 20px; margin-right: 5px;">` + "수정" + "</button>";
				let deleteBtn = `<button type=button id=deleteBtn value=${reviewId} class="btn btn-sm btn-secondary">` + "삭제" + "</button>";
